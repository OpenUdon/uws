...
| Define and validate Browser 1.9 text-sink contract | `[ ]` | Owns N7/N17 and the new-profile portion of N1. Publish deterministic literal-brace escaping, URL encoding, prompt control/bidi policy, `type_text` value-setting semantics, BCP 14 and safe-integer rules in a new schema/spec with focused positive/negative fixtures. |
| Integrate and verify Browser 1.9 distribution | `[ ]` | Update explicit lookup/validation, embedded archive and hash protections, fixtures, README, guides/navigation, changelog, AGENTS and current memory-bank facts where applicable. Test Browser 1.5–1.8 compatibility, full acceptance commands, and whole-milestone review before completion. |
