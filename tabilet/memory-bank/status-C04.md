...
| Align expression grammar, numeric literals, and lint | `[ ]` | N4/N5/N20/F01/F03. Add 1.11 `$response.body.<path>`, loop-only `$batchIndex`, and complete JSON-number forms for numeric `wait`/`batchSize`; make content-trust parsing version-aware. Lint docs/fixtures by declared version; retain old fixture versions with exact-path documented legacy/§5.5 allowlist entries and no conformance claim, and add conforming 1.11 fixtures. |
