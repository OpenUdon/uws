...
A single document with three operation kinds — OpenAPI-bound, function call, and native source-bound:
...
operations:
  ...
  # Shape 2: source-bound with the generic selector
...
Weather and Gmail stay source-bound. Email formatting is runtime-owned. The document validates before any execution begins.
...
output capabilities, provenance inheritance, and any references it extracted.
Without one, the analyzer reports opaque or unknown flow; ordinary validation
still accepts the profile and execution behavior is unchanged.
