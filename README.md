...
UWS 1.10.0 is the latest release. It keeps the UWS 1.x wire model and publishes portable execution semantics for expressions, delays, structural results, actions, and trigger dispatch, while retaining the 1.9.1 content-provenance model. UWS 1.10 also rejects expression-addressable names that its grammar cannot reference; earlier documents retain their existing behavior. Browser 1.8 remains a separate profile. The `ansible-module` source type added in 1.6 was removed in 1.7, and UWS 1.9+ defines no replacement Ansible operation profile. Missing `sourceDescription.type` still defaults to `openapi`; legacy `openapiOperationId` and `openapiOperationRef` remain valid for OpenAPI sources.
