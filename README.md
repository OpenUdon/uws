...
UWS 1.9.2 is the latest release. It keeps OpenAPI compatibility and nine source description types, retains the 1.9.1 content-provenance model, and adds compatible validation and advisory-analysis corrections. The `ansible-module` source type added in 1.6 was removed in 1.7, and UWS 1.9 defines no replacement Ansible operation profile. Missing `sourceDescription.type` still defaults to `openapi`; legacy `openapiOperationId` and `openapiOperationRef` remain valid for OpenAPI sources.
...
- Human-readable specification: [versions/1.9.2.md](versions/1.9.2.md)
...
- JSON Schema: [versions/1.9.2.json](versions/1.9.2.json)
...
`versions/1.9.2.json` provides structural JSON Schema validation. Use the Go validator for semantic checks such as duplicate identifiers, reference integrity, and malformed `contentTrust` declarations. Go callers resolve it with `schemas.PathForVersion`.
