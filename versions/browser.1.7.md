# UWS Browser Capability Profile 1.7
...
| Field | Value |
| --- | --- |
| Profile name | `uws.browser.1.7` |
| JSON Schema | `versions/browser.1.7.json` |
...
| Field | Type | Purpose |
| --- | --- | --- |
| `profile` | string const | REQUIRED. Literal `uws.browser.1.7`. |
...
Browser 1.7 adds portable popup and frame relationships while retaining every
...
`null` are invalid browser 1.7 documents.
...
The following are NOT part of `browser.1.7` and MUST NOT appear in the
profile document:
...
`uws.browser.1.5` and `uws.browser.1.6` remain immutable and accepted. A
