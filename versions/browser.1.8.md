# UWS Browser Capability Profile 1.8

The UWS Browser Capability Profile is the sub-spec for documents referenced by
UWS 1.9 `sourceDescriptions[].type: browser-profile`. It is wire/spec metadata
...
`sourceOperationRef` selector rules. Browser 1.8 retains the browser 1.7
capability contract and defines parameter substitution contexts and scalar
formatting. Browser-aware tooling validates profile structure and template
references; runtimes validate concrete inputs and perform substitution.
...
| Field | Value |
| --- | --- |
| Profile name | `uws.browser.1.8` |
| JSON Schema | `versions/browser.1.8.json` |
...
| Field | Type | Purpose |
| --- | --- | --- |
| `profile` | string const | REQUIRED. Literal `uws.browser.1.8`. |
...
Browser 1.8 retains browser 1.7's portable popup and frame relationships and
browser 1.5's main-page action shapes. Omitted `context` means the implicit
`main` page.
...
| Macro | Shape | Purpose |
| --- | --- | --- |
| `navigate` | string or `{ url, context? }` | Relative or absolute URL. Browser 1.8 templates are allowed only in path segments and query values; runtimes encode substituted values as RFC 3986 components and reject unsafe dot segments. Runtime MUST reject targets outside `info.origin`, context/origin mismatches, and a relative target when `info.origin` contains more than one origin. |
...
| `type_text` | `{ locator, value, wait_for?, context? }` | Input text. Browser 1.8 permits literal scalar substitution in `value`. |
...
| `select_option` | `{ locator, value, wait_for?, context? }` | Pick a value option from a drop-down. Browser 1.8 permits literal scalar substitution in `value`. |
...
`null` are invalid browser 1.8 documents.
...
## Parameters, Safe Substitution, and Formatting

Each action's `parameters` field is an inline JSON Schema. A placeholder MUST
use the exact form `{{name}}`, where `name` matches
`[A-Za-z][A-Za-z0-9_-]*`, and MUST name a declared property. Nested
substitution, runtime expressions, whitespace in the braces, unmatched
double braces, and undeclared names are invalid. Substitution is one pass:
inserted values are not rescanned for more placeholders.

Browser 1.8 permits placeholders only in these positions:

| Sink | Placement | Substitution |
| --- | --- | --- |
| `navigate` URL | Within a path segment, or within a query value after `=` and before the next `&` or `#`. | Percent-encode the parameter's UTF-8 bytes as an RFC 3986 component, preserving only ASCII letters, digits, `-`, `.`, `_`, and `~`; use uppercase hex escapes. Path and query values use the same component encoding. |
| `type_text.value`, `select_option.value` | Any position in the value string. | Insert the canonical scalar text below, without URI or HTML encoding. These are browser input values, not markup. |
| `confirmationPolicy.prompt` | Any position in the prompt. | Insert the canonical scalar text below as literal human-readable text; prompt renderers MUST treat it as text, not markup. |

Templates MUST NOT appear in a URL scheme, authority, query name, fragment,
locator, wait condition, context identifier, output declaration, or any other
field. Query delimiters are static: a placeholder cannot introduce a key,
separator, or additional query parameter. Malformed or misplaced templates
MUST fail validation rather than be interpreted differently by runtimes.
...
| `integer` | A signed 64-bit integer in canonical base-10 form: `0` or an optional minus followed by a non-zero digit and digits. No plus sign or leading zero is allowed. |
...
Schema `default` values MUST also satisfy their parameter schema. When a
caller omits a parameter, a valid default is used; if it is absent and the
parameter is required, the runtime MUST fail the action before executing any
sequence step.

For example, a string `a/b?c` in `/records/{{id}}` becomes
`/records/a%2Fb%3Fc`; the same string in `?q={{query}}` becomes
`?q=a%2Fb%3Fc`. In `type_text.value` it remains `a/b?c`. The number
`1.0` is inserted as `1`, and Boolean false as `false`.

Browser 1.5, 1.6, and 1.7 retain their original substitution behavior. A
consumer MUST select `uws.browser.1.8` explicitly to claim this contract.
...
The following are NOT part of `browser.1.8` and MUST NOT appear in the
profile document:
...
`sourceOperationId` / `sourceOperationRef` per UWS 1.9.

## Compatibility

`uws.browser.1.5`, `uws.browser.1.6`, and `uws.browser.1.7` remain
immutable and accepted. A main-only profile that does not need
context-qualified steps or outputs SHOULD
...
1.6 with UWS 1.8. Browser 1.7 and 1.8 require UWS 1.9; the concrete
browser-driver protocol remains a downstream runtime implementation detail.
