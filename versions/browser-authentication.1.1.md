# UWS Browser Authentication Profile 1.1

`uws.browser-authentication.1.1` is an additive, inert profile for reviewed
browser sign-in recipes. It does not revise authentication 1.0 or browser 1.5;
existing profiles and workflows remain valid and unchanged.
...
The profile supports sign-in only. Enrollment, recovery, password
...
flows:
  member_login_push:
    sequence:
      ...
      - click:
          locator: {role: button, name: Sign in with identity provider}
      ...
      - click:
          ...
          context: idp_popup
      - challenge: {kind: push, context: idp_popup}
      - wait_for:
...
declared origin. Its optional `path` is an exact clean path without query or
fragment.

The optional `contexts` map follows browser 1.6: `main` is implicit, IDs are
bounded, parents form an acyclic graph at most four levels deep, and every
context origin is declared by `info`. Frames require an exact clean path or
name and must resolve uniquely. Popups are opened by exactly one explicit
click using `opensContext`; the click context must match the popup parent.
...
| Step | Purpose |
| --- | --- |
| `navigate` | Open an absolute URL on a declared origin; the object form `{url, context?}` targets a declared context. |
| `type_credential` | Fill one accessibility locator from one declared slot, optionally in `context`. |
| `click` | Activate one accessibility locator, optionally in `context` and opening one `opensContext` popup. |
| `challenge` | Perform one explicitly selected MFA handoff, optionally in `context`. |
| `wait_for` | Wait for one accessibility locator, optionally in `context`. |
...
`uws.browser-authentication.1.0` remains immutable and accepted. Main-only
recipes without an exact success path SHOULD keep 1.0. Contexts, context-
qualified steps, the navigate object form, and success `path` require 1.1.
Validators select the schema from the exact `profile` discriminator and reject
unknown versions.
