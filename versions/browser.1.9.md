# UWS Browser Capability Profile 1.9
...
`sourceOperationRef` selector rules. UWS 1.10.0 is the current core release;
Browser 1.9 does not require a core schema change. Browser 1.9 retains the
Browser 1.8 capability contract and adds deterministic literal-brace escapes,
safe text-sink behavior, and a consistent safe-integer boundary.
Browser-aware tooling validates profile structure and template references;
runtimes validate concrete inputs and perform substitution.
...
| Field | Value |
| --- | --- |
| Profile name | `uws.browser.1.9` |
| JSON Schema | `versions/browser.1.9.json` |
...
| Field | Type | Purpose |
| --- | --- | --- |
| `profile` | string const | REQUIRED. Literal `uws.browser.1.9`. |
...
consumer MUST select `uws.browser.1.9` explicitly to claim this contract.
...
The following are NOT part of `browser.1.9` and MUST NOT appear in the
profile document:
...
[current UWS 1.10.0 specification](1.10.0.md) for the core binding contract.

## Compatibility

`uws.browser.1.5` through `uws.browser.1.8` remain immutable and accepted.
Browser 1.9 is opt-in; consumers MUST select its exact discriminator to claim
the escape and safe-text contract. A main-only profile that does not need
...
use the generic browser-profile binding introduced in UWS 1.9; Browser 1.9 is
also usable with the current UWS 1.10.0 core. The concrete browser-driver
